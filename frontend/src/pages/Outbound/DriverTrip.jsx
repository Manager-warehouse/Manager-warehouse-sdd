...
const DELIVERY_STATUS_MAP = {
  ...
  COMPLETED: {
    label: "Đã giao",
    color: "bg-success-50 text-success-900 border-success-300",
  },
  RETURNED: {
    label: "Chờ hoàn về kho",
...
const isDeliveryStopTerminal = (status) =>
  TERMINAL_DELIVERY_STATUSES.has(status);

const StatusBadge = ({ status }) => {
  const { label, color } = DELIVERY_STATUS_MAP[status] ?? {
    label: status,
    color: "bg-canvas-cream text-shade-70 border-hairline-light",
  };
  return (
    <Badge size="sm" colorClassName={color}>
      {label}
    </Badge>
  );
};
...
const getTripType = (trip = {}) =>
  trip.trip_type || trip.tripType || trip.type || "DELIVERY";
const isTransfer = (trip = {}) => getTripType(trip) === "TRANSFER";
const typeLabel = (trip = {}) =>
  trip.trip_type_label ||
  trip.tripTypeLabel ||
  (isTransfer(trip) ? "Dieu chuyen noi bo" : "Giao dai ly");
const getTransferId = (trip = {}) => trip.transfer_id || trip.transferId;
const getSourceWarehouseCode = (trip = {}) =>
  trip.source_warehouse_code || trip.sourceWarehouseCode;
const getDestinationWarehouseCode = (trip = {}) =>
  trip.destination_warehouse_code || trip.destinationWarehouseCode;
...
function OTPCountdown({ expiresAt, onExpired }) {
  ...
  return (
    <span
      className={`font-mono font-bold text-sm ${remaining < 60 ? "text-danger-600" : "text-shade-50"}`}
    >
      {mm}:{ss}
    </span>
  );
...
export default function DriverTrip() {
  ...
  const fetchTrips = async () => {
    ...
    try {
      ...
      const transferTrips = transfers
        .filter(
          (transfer) =>
            transfer.tripId &&
            Number(transfer.driverUserId || 0) === Number(user?.id || 0),
        )
      ...
      loadedTrips.push(
        ...transferTrips.filter(
          (tripRow) =>
            !existingTransferKeys.has(
              String(
                getTransferId(tripRow) || tripRow.trip_id || tripRow.tripId,
              ),
            ),
        ),
      );
    ...
    try {
      ...
      setTrips(
        loadedTrips.sort(
          (a, b) =>
            new Date(b.planned_date || 0) - new Date(a.planned_date || 0),
        ),
      );
    } finally {
      setLoading(false);
    ...
    try {
      if (String(tripId).startsWith(TRANSFER_TRIP_PREFIX)) {
        const transferId = Number(
          String(tripId).replace(TRANSFER_TRIP_PREFIX, ""),
        );
        const data =
          await interWarehouseTransferService.getTransferById(transferId);
    ...
    try {
      ...
      const messages = {
        ...
        ASSIGNED_DRIVER_REQUIRED:
          "Chuyến này không được phân công cho tài xế hiện tại",
  ...
  const handleUploadPodAndRequestOTP = async () => {
    if (!goodsImage || !signDocumentImage) {
      addToast(
        "Vui lòng tải đủ ảnh hàng hóa và ảnh biên bản giao nhận",
        "error",
      );
      return;
    }
    setSubmitting(true);
    try {
      await outboundService.uploadPodEvidence(trip.id, activeDO.do_id, {
        goodsImage,
        signDocumentImage,
        notes,
      });
      const otp = await outboundService.requestOTP(
        trip.id,
        activeDO.do_id,
        otpSent,
      );
  ...
  const handleVerifyOTP = async (code) => {
    setSubmitting(true);
    try {
      await outboundService.verifyOTPAndDeliver(
        trip.id,
        activeDO.do_id,
        code,
        notes,
      ...
      <div className="flex flex-col gap-6">
        <div>
          ...
          <p className="text-xs text-shade-50 font-light mt-1">
            Các chuyến xe được phân công cho tài xế hiện tại.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-canvas-light rounded-lg border border-hairline-light p-12 text-center shadow-level-3">
            ...
            <p className="text-sm text-shade-50">
              {trips.length
                ? "Không có chuyến xe phù hợp với bộ lọc hiện tại."
                : "Hiện tại bạn chưa được gán chuyến xe nào."}
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredTrips.map((tripItem) => (
              <div
                ...
                onClick={() => {
                  const detailId =
                    isTransfer(tripItem) && getTransferId(tripItem)
                      ? `${TRANSFER_TRIP_PREFIX}${getTransferId(tripItem)}`
                      : tripItem.id;
                  if (!detailId) {
                    addToast(
                      "Chuyến xe chưa có mã nội bộ để mở chi tiết",
                      "warning",
                    );
                    return;
                  }
              ...
              >
                <div className="p-4 border-b border-hairline-light bg-canvas-cream flex justify-between items-center">
                  <div className="min-w-0">
                    <span className="text-xs font-bold text-ink block truncate">
                      {tripItem.trip_number}
                    </span>
                    <span className="text-[10px] font-semibold text-shade-50 uppercase tracking-wider">
                      {typeLabel(tripItem)}
                    </span>
                  </div>
                ...
                <div className="p-4 flex flex-col gap-2 text-xs">
                  <p className="flex items-center gap-2 text-shade-50">
                    <Truck className="w-3.5 h-3.5 text-shade-40" /> Xe:{" "}
                    <span className="font-semibold text-ink">
                      {tripItem.vehicle_plate || "-"}
                    </span>
                  </p>
                  <p className="flex items-center gap-2 text-shade-50">
                    <Calendar className="w-3.5 h-3.5 text-shade-40" /> T.gian dự
                    kiến:{" "}
                    <span className="font-semibold text-ink">
                      {tripItem.planned_start_at
                        ? new Date(tripItem.planned_start_at).toLocaleString(
                            "vi-VN",
                            {
                              hour: "2-digit",
                              minute: "2-digit",
                  ...
                  {isTransfer(tripItem) ? (
                    <>
                      <p className="flex items-center gap-2 text-shade-50">
                        <MapPin className="w-3.5 h-3.5 text-shade-40" /> Tuyến:{" "}
                        <span className="font-semibold text-ink">
                          {getSourceWarehouseCode(tripItem) || "-"} →{" "}
                          {getDestinationWarehouseCode(tripItem) || "-"}
                        </span>
                      </p>
                      <p className="flex items-center gap-2 text-shade-50">
                        <MapPin className="w-3.5 h-3.5 text-shade-40" /> Dòng
                        hàng:{" "}
                        <span className="font-semibold text-ink">
                          {tripItem.transfer_line_count ??
                    ...
                    <p className="flex items-center gap-2 text-shade-50">
                      <MapPin className="w-3.5 h-3.5 text-shade-40" /> Điểm
                      giao:{" "}
                      <span className="font-semibold text-ink">
                        {tripItem.delivery_stop_count ??
                          tripItem.delivery_orders?.length ??
                          0}
                      </span>
                    </p>
                  )}
                  <p className="text-xs text-shade-50 pt-1">
                    Tổng KL:{" "}
                    <span className="font-semibold text-ink">
                      {tripItem.total_weight_kg || 0} kg
                    </span>
                  </p>
                </div>
              </div>
  ...
  const processedStopCount = deliveryOrders.filter((d) =>
    isDeliveryStopTerminal(d.delivery_status),
  ).length;
  const totalCount = isTransferTrip
    ? (trip.items?.length ?? trip.transfer_line_count ?? 0)
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-start gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-shade-50 font-light mt-1">
            {isTransferTrip
              ? `${getSourceWarehouseCode(trip)} → ${getDestinationWarehouseCode(trip)}`
              : `${processedStopCount}/${totalCount} điểm đã xử lý`}
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start">
        <div className="w-full lg:w-80 shrink-0 p-5 card-premium rounded-lg">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xs font-bold uppercase tracking-wider text-shade-40">
              Thông tin chuyến
            </h2>
          </div>
          <div className="flex flex-col gap-2 text-xs mb-4">
            <p className="flex items-center gap-2 text-shade-50">
              <Truck className="w-3.5 h-3.5 text-shade-40 shrink-0" /> Xe:{" "}
              <span className="font-semibold text-ink">
                {trip.vehicle_plate}
              </span>
            </p>
            <p className="flex items-center gap-2 text-shade-50">
              <User className="w-3.5 h-3.5 text-shade-40 shrink-0" /> Tài xế:{" "}
              <span className="font-semibold text-ink">{trip.driver_name}</span>
            </p>
            <p className="flex items-center gap-2 text-shade-50">
              <Calendar className="w-3.5 h-3.5 text-shade-40 shrink-0" /> Khởi
              hành:{" "}
              <span className="font-semibold text-ink">
                {new Date(trip.planned_date).toLocaleString("vi-VN")}
              </span>
            </p>
            {trip.planned_end_at && (
              <p className="flex items-center gap-2 text-shade-50">
                <Calendar className="w-3.5 h-3.5 text-shade-40 shrink-0" /> Hạn
                giao:{" "}
                <span className="font-semibold text-ink">
                  {new Date(trip.planned_end_at).toLocaleString("vi-VN")}
                </span>
              </p>
            )}
          </div>
            ...
            <>
              ...
              {!transferLoaded && (
                <p className="mt-2 text-[11px] leading-relaxed text-warning-700">
                  Thủ kho nguồn cần xác nhận xếp đủ số lượng hàng lên xe trước
                  khi tài xế xuất phát.
                </p>
              )}
            </>
          )}
              ...
              <div className="mt-4 pt-4 border-t border-hairline-light">
                {!trip.driverArrivedAt ? (
                  ...
                  <p className="text-[11px] leading-relaxed text-success-700 font-semibold">
                    Đã xác nhận đến kho đích. Chờ kho bàn giao hàng.
                  </p>
                )}
              </div>
            )}
              ...
              <div className="mt-4 pt-4 border-t border-hairline-light">
                <div className="bg-danger-50 border border-danger-200 rounded-lg p-3 text-xs text-danger-700 mb-3">
                  <p className="font-bold">
                    Quản lý kho đã yêu cầu quay đầu về kho nguồn.
                  </p>
                  {trip.returnReason && (
                    <p className="mt-1">Lý do: {trip.returnReason}</p>
                  )}
                </div>
                ...
                {trip.returnArrivedAt && !trip.returnArrivalHandoverAt && (
                  <p className="text-[11px] leading-relaxed text-warning-700 font-semibold">
                    Đã về kho nguồn. Chờ kho nguồn nhận bàn giao để nhân viên
                    đếm hàng.
                  </p>
                )}
                {trip.returnArrivalHandoverAt && (
                  <p className="text-[11px] leading-relaxed text-success-700 font-semibold">
                    Kho nguồn đã nhận bàn giao. Chờ nhân viên đếm và thủ kho QC
                    trước khi nhập lại tồn.
                  </p>
                )}
              </div>
            )}
            ...
            <div className="mt-4 pt-4 border-t border-hairline-light">
              ...
              <div className="w-full bg-canvas-cream rounded-full h-2">
                <div
                  ...
                  style={{
                    width: `${totalCount > 0 ? (processedStopCount / totalCount) * 100 : 0}%`,
                  }}
                />
              </div>
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-hairline-light">
              <div className="bg-success-50 border border-success-300 rounded-lg p-3 text-xs text-success-900 flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-success-800 mt-0.5">
                    Xe và tài xế đã được xác nhận về kho an toàn.
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="flex-1 flex flex-col gap-3">
          <h3 className="text-xs font-bold uppercase tracking-wider text-shade-40">
            {isTransferTrip
              ? `Danh sách hàng điều chuyển (${totalCount} dòng)`
          ...
          {isTransferTrip &&
            (trip.items || []).map((item) => (
              <div
                key={item.id}
                className="rounded-lg border border-hairline-light bg-canvas-light shadow-level-3 p-4"
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <h4 className="text-sm font-bold text-ink leading-tight">
                      {item.productSku}{" "}
                      <span className="font-normal text-shade-50">
                        {item.productName}
                      </span>
                    </h4>
                    <p className="text-xs text-shade-50 mt-1">
                      Kế hoạch: {item.plannedQty} · Xuất: {item.sentQty ?? "-"}
                    </p>
                  </div>
          ...
          {!isTransferTrip &&
            trip.delivery_orders?.map((doItem, index) => {
              ...
              return (
                <div
                  key={`${doItem.do_id}-${index}`}
                  className={`rounded-lg border overflow-hidden ${isDelivered ? "bg-success-50 border-success-300" : isUnsuccessful ? "bg-danger-50 border-danger-200" : "bg-canvas-light border-hairline-light shadow-level-3"}`}
                >
                  <div className="p-4 flex gap-3">
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 font-bold text-sm ${isDelivered ? "bg-success-600 text-white" : isUnsuccessful ? "bg-danger-500 text-white" : "bg-ink text-white"}`}
                    >
                      {isDelivered ? (
                        <CheckCircle2 className="w-4 h-4" />
                      ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start gap-2">
                        <h4 className="text-sm font-bold text-ink leading-tight">
                          {doItem.dealer_name || doItem.do_number}
                        </h4>
                      ...
                      {isPending && trip.status === "IN_TRANSIT" &&
                        ...
                        <div className="flex gap-2 mt-4">
                          ...
                          <button
                            ...
                            className="px-3.5 py-2.5 rounded-pill bg-danger-50 text-danger-700 border border-danger-200 hover:bg-danger-100 active:scale-95 transition-all"
                      ...
                      {isUnsuccessful && doItem.failure_reason && (
                        <p className="text-xs text-danger-600 mt-2 flex items-center gap-1">
                          <X className="w-3.5 h-3.5 shrink-0" /> Thất bại:{" "}
                          {doItem.failure_reason}
                        </p>
                      )}
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 bg-canvas-night/60 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-canvas-light w-full sm:max-w-md rounded-t-2xl sm:rounded-xl max-h-[90vh] flex flex-col shadow-level-4">
            <div className="px-5 py-4 border-b border-hairline-light flex items-center justify-between">
              ...
              <button
                onClick={closeModal}
                className="p-1.5 hover:bg-canvas-cream rounded-full text-shade-50 hover:text-ink transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-5 overflow-y-auto flex-1 flex flex-col gap-5">
              ...
              <textarea
                className="text-input text-sm h-20 resize-none"
              ...
              <div className="border-t border-hairline-light pt-5">
                ...
                {!otpSent ? (
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    {otpExpired && (
                      <p className="text-xs text-danger-600 text-center font-medium">
                        Mã OTP đã hết hạn.
                      </p>
                    )}
                    <button
      ...
      {activeDO && modalType === "FAIL" && (
        <div className="fixed inset-0 bg-canvas-night/60 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-canvas-light w-full sm:max-w-md rounded-t-2xl sm:rounded-xl max-h-[90vh] flex flex-col shadow-level-4">
            <div className="px-5 py-4 border-b border-hairline-light flex items-center justify-between">
              <h3 className="text-base font-bold text-danger-600 flex items-center gap-2">
            ...
            <div className="p-5 overflow-y-auto flex-1 flex flex-col gap-5">
              <div className="bg-danger-50 border border-danger-100 rounded-lg p-3 text-sm text-danger-800">
                Đơn <strong>{activeDO.do_number}</strong>
              </div>
              <div>
                ...
                <textarea
                  ...
                  placeholder="Đại lý đóng cửa, từ chối nhận..."
                  value={failureReason}
              ...
              <button
                ...
                disabled={!failureReason.trim() || submitting}
                className="w-full py-3.5 bg-danger-600 text-white font-bold rounded-pill text-sm hover:bg-danger-700 disabled:opacity-50 active:scale-95 transition-all"
              >
                {submitting ? (
