...
const InterWarehouseTransferWorkspace = () => {
  ...
  useEffect(() => {
    ...
    const loadAvailability = async () => {
      ...
      const requests = form.items.map(async (item, index) => {
        ...
        try {
          const availability =
            await interWarehouseTransferService.getAvailability(
              sourceWarehouseId,
              productId,
            );
    ...
    try {
      ...
      setWarehouses(
        warehouseRows.filter(
          (warehouse) =>
            isActiveRecord(warehouse) && isPhysicalWarehouse(warehouse),
        ),
      );
      ...
      setSelectedId((current) =>
        requestedId &&
        transferRows.some((transfer) => transfer.id === requestedId)
          ? requestedId
  ...
  useEffect(() => {
    ...
    const outboundQcDone =
      selectedTransfer.outboundQcPassed !== null &&
    ...
    const loadSelectedAvailability = async () => {
      ...
      const requests = selectedTransfer.items.map(async (item) => {
        try {
          ...
          return [
            item.id,
            { error: error.message || "Không tải được tồn kho" },
          ];
        }
      });
  ...
  const filteredTransfers = useMemo(
    () =>
      visibleTransfers.filter((transfer) => {
        const haystack =
          `${transfer.transferNumber} ${transfer.externalInstructionCode} ${transfer.sourceWarehouseCode} ${transfer.destinationWarehouseCode}`.toLowerCase();
        return (
  ...
  const sourceWarehouseOptions = useMemo(() => {
    ...
    return [
      ...
      ...list.map((w) => ({ value: w.id, label: `${w.code} - ${w.name}` })),
    ];
  ...
  const createTransfer = async () => {
    try {
      ...
      const payload = {
        ...
        items: form.items.map((item, index) => {
          ...
          if (
            availability &&
            !availability.error &&
            Number(item.plannedQty) > Number(availability.availableQty ?? 0)
          ) {
            throw new Error(
              `Số lượng đặt của ${product.sku} vượt tồn khả dụng`,
            );
          }
          return {
      ...
      const created =
        await interWarehouseTransferService.createTransfer(payload);
    ...
    try {
      ...
      const actions = {
        ...
        recordSourceLoadReport: () =>
          interWarehouseTransferService.recordSourceLoadReport(
            id,
            payload.items,
            payload.reworkReason,
          ),
        ship: () => interWarehouseTransferService.shipTransfer(id),
        unship: () => interWarehouseTransferService.unshipTransfer(id),
        recordOutboundQc: () =>
          interWarehouseTransferService.recordOutboundQc(id, payload),
        loadHandover: () =>
          interWarehouseTransferService.loadHandover(id, payload),
        depart: () => interWarehouseTransferService.departTransfer(id),
        driverArrive: () => interWarehouseTransferService.driverArrive(id),
        receivingHandover: () =>
          interWarehouseTransferService.receivingHandover(id, payload),
        receiveCount: () =>
          interWarehouseTransferService.receiveCount(id, payload),
        receiveCheck: () =>
          interWarehouseTransferService.receiveCheck(id, payload),
        finalReceive: () =>
          interWarehouseTransferService.finalReceive(id, payload),
        returnToSource: () =>
    ...
    <div className="mobile-page">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 md:gap-4 flex-wrap">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-shade-50 font-light mt-1">
            Lập phiếu thủ công từ lệnh Công ty mẹ, giữ chỗ, xuất hàng, vận
        ...
        <div className="mobile-filter-bar sm:flex sm:gap-2 sm:flex-shrink-0">
          ...
          {canCreateTransfer && (
            <Button icon={Plus} onClick={() => setFormOpen((value) => !value)}>
              Tạo phiếu
            </Button>
          )}
        </div>
      ...
      {formOpen && (
        <div className="border border-hairline-light rounded-lg bg-canvas-light p-3 md:p-4 grid grid-cols-1 md:grid-cols-4 gap-3">
          <Input
            label="Mã lệnh Công ty mẹ"
            value={form.externalInstructionCode}
            onChange={(e) =>
              setForm({ ...form, externalInstructionCode: e.target.value })
            }
          />
          <Input
            type="select"
          ...
          <div className="md:col-span-4 flex flex-col gap-2">
            {form.items.map((item, index) => (
              <div
                key={index}
                className="grid grid-cols-1 md:grid-cols-[1fr_160px_160px_120px] gap-2 items-end"
              >
                <Input
                  type="select"
                  label="Sản phẩm"
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setItem(index, { plannedQty: e.target.value })
                  }
                />
                <div className="min-h-[44px] rounded-md border border-hairline-light bg-canvas-cream/50 px-3 py-2 text-xs">
                  <div className="font-semibold uppercase tracking-wider text-shade-60">
                    Tồn khả dụng
                  </div>
                  {availabilityByLine[index]?.error ? (
                    ...
                    <div
                      className={
                        ...
                        Number(availabilityByLine[index].availableQty ?? 0)
                          ? "text-danger-600 font-semibold"
                          : "text-ink"
                      }
                    >
                      {availabilityByLine[index].availableQty ?? 0}
                      <span className="text-shade-50 font-normal">
                        {" "}
                        / giữ {availabilityByLine[index].reservedQty ?? 0}
                      </span>
                    </div>
                  ) : (
            ...
            <div className="flex gap-2">
              <Button
                ...
                onClick={() =>
                  setForm((current) => ({
                    ...current,
                    items: [...current.items, { productId: "", plannedQty: 1 }],
                  }))
                }
              >
                Thêm dòng
            ...
            <div className="flex flex-col md:flex-row gap-4 items-center w-full md:w-auto">
              <div className="w-full md:w-80">
                <Input
                  ...
                  placeholder="Tìm mã phiếu, mã lệnh..."
                />
              </div>
              <Input
                type="select"
                ...
                options={[
                  ...
                  { value: "IN_TRANSIT", label: "IN_TRANSIT" },
                  {
                    ...
                    label: "PUTAWAY_PENDING_APPROVAL",
                  },
                  { value: "COMPLETED", label: "COMPLETED" },
                  {
                    value: "COMPLETED_WITH_DISCREPANCY",
                    label: "COMPLETED_WITH_DISCREPANCY",
                  },
                      ...
                      >
                        <td className="px-6 py-4">
                          ...
                          <div className="text-xs text-shade-50">
                            {transfer.externalInstructionCode}
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-4">
                          <InterWarehouseTransferStatusBadge
          ...
          {selectedTransfer && (
            <div className="border border-hairline-light rounded-lg bg-canvas-light p-4">
              <div className="flex items-center justify-between mb-3">
                <div>
                  <div className="text-xs font-bold uppercase tracking-wider text-shade-60">
                    Chi tiết hàng
                  </div>
                  <div className="font-semibold">
              ...
              {selectedTransfer.tripId && (
                <div className="mb-3 grid grid-cols-1 md:grid-cols-3 gap-2 rounded-md border border-hairline-light bg-canvas-cream/50 px-3 py-2 text-xs">
                  <span>
                    <span className="text-shade-50">Chuyến:</span>{" "}
                    <strong>{selectedTransfer.tripNumber || "-"}</strong>
                  </span>
                  <span>
                    <span className="text-shade-50">Xe:</span>{" "}
                    <strong>{selectedTransfer.vehiclePlate || "-"}</strong>
                  </span>
                  <span>
                    <span className="text-shade-50">Tài xế:</span>{" "}
                    <strong>{selectedTransfer.driverName || "-"}</strong>
                  </span>
                  <span>
                    <span className="text-shade-50">Bắt đầu:</span>{" "}
                    <strong>
                      {selectedTransfer.tripPlannedStartAt
                        ? new Date(
                            selectedTransfer.tripPlannedStartAt,
                  ...
                  <span>
                    <span className="text-shade-50">Hạn giao:</span>{" "}
                    <strong>
                      {selectedTransfer.tripPlannedEndAt
                        ? new Date(
                            selectedTransfer.tripPlannedEndAt,
                          ).toLocaleString("vi-VN")
                        : "-"}
              ...
              {selectedTransfer.status === "COMPLETED_WITH_DISCREPANCY" &&
                  ...
                  <div className="mb-3 rounded-md border border-danger-200 bg-danger-50/80 px-3.5 py-2.5 text-xs text-danger-700 shadow-level-3 flex items-start gap-2 animate-in fade-in duration-200">
                    ...
                    <div>
                      <span className="font-bold block mb-0.5">
                        Phát hiện chênh lệch nhận hàng
                      </span>
                      <span className="font-normal text-danger-600">
                        Lý do: "{selectedTransfer.discrepancyReason}"
                      </span>
                    </div>
                  </div>
                )}
              <div className="divide-y divide-hairline-light">
                {selectedTransfer.items?.map((item) => {
                  const sent =
                    item.sentQty !== null ? Number(item.sentQty) : null;
                  const received =
                    item.receivedQty !== null ? Number(item.receivedQty) : null;
                  const hasDiscrepancy =
                    sent !== null && received !== null && received - sent !== 0;
                  const diff =
                    sent !== null && received !== null ? received - sent : 0;
                  ...
                  return (
                    <div key={item.id} className="py-2.5 text-xs">
                      <div className="font-semibold text-sm">
                        {item.productSku}{" "}
                        <span className="font-normal text-shade-60">
                          {item.productName}
                        </span>
                      </div>
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-2 mt-2 bg-canvas-cream p-2 rounded border border-hairline-light">
                        <div>
                          <span className="text-shade-50">Kế hoạch:</span>{" "}
                          <strong>{item.plannedQty}</strong>
                        </div>
                        <div
                          className={
                            Number(item.plannedQty) >
                            Number(
                              selectedAvailabilityByItem[item.id]
                                ?.availableQty ?? 0,
                            )
                              ? "text-danger-600 font-semibold"
                              : ""
                        ...
                        >
                          <span className="text-shade-50">Khả dụng nguồn:</span>{" "}
                          <strong>
                            {selectedAvailabilityByItem[item.id]?.error
                              ? "Lỗi tải"
                              : (selectedAvailabilityByItem[item.id]
                                  ?.availableQty ?? "-")}
                          </strong>
                        ...
                        <div>
                          <span className="text-shade-50">Đã xuất đi:</span>{" "}
                          <strong className="text-ink">
                            {item.sentQty ?? "-"}
                          </strong>
                        </div>
                        <div>
                          <span className="text-shade-50">Thực tế nhận:</span>{" "}
                          <strong className="text-success-700">
                            {item.receivedQty ?? "-"}
                          </strong>
                        </div>
                        <div>
                          <span className="text-shade-50">QC đạt/lỗi:</span>{" "}
                          <strong>
                            {item.qcPassedQty ?? "0"} /{" "}
                            {item.qcFailedQty ?? "0"}
                          </strong>
