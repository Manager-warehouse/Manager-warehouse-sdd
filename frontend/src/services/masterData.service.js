...
const INITIAL_WAREHOUSES = [
  { id: 1, code: 'HP-01', name: 'Kho Hải Phòng', address: 'Số 1 Lê Thánh Tông, Ngô Quyền, Hải Phòng', phone: '02253888999', manager_id: 3, type: 'PHYSICAL', is_active: true },
  { id: 2, code: 'HN-01', name: 'Kho Hà Nội', address: 'Số 15 Cầu Giấy, Quan Hoa, Hà Nội', phone: '0243111222', manager_id: 4, type: 'PHYSICAL', is_active: true },
  { id: 3, code: 'HCM-01', name: 'Kho Hồ Chí Minh', address: 'Số 100 Cộng Hòa, Tân Bình, TP. HCM', phone: '0283555666', manager_id: 2, type: 'PHYSICAL', is_active: true },
  { id: 4, code: 'IN_TRANSIT', name: 'Kho Trung Chuyển Nội Bộ', address: 'Trên đường vận chuyển', phone: 'N/A', manager_id: 1, type: 'IN_TRANSIT', is_active: true },
];
...
const INITIAL_LOCATIONS = [
  {
    ...
    capacity_m3: 5.000,
    capacity_kg: 1000.00,
    current_volume_m3: 2.150,
    current_weight_kg: 350.00,
    is_quarantine: false,
    is_active: true
  },
  {
  ...
  },
  {
    ...
    capacity_m3: 8.000,
    capacity_kg: 1500.00,
    current_volume_m3: 4.500,
    current_weight_kg: 900.00,
    is_quarantine: false,
    is_active: true
  }
];
...
export const masterDataService = {
  ...
  createProduct: async (productData) => {
    if (useMock) {
      ...
      const exists = products.some(p => p.sku.trim().toLowerCase() === productData.sku.trim().toLowerCase());
      ...
      addMockAuditLog('PRODUCT_CREATED', 'Product', newProduct.id, `Tạo mới SKU: ${newProduct.sku} (${newProduct.name})`);
      return newProduct;
    }
  ...
  updateProduct: async (id, productData) => {
    if (useMock) {
      ...
      const skuExists = products.some(p => p.id !== Number(id) && p.sku.trim().toLowerCase() === productData.sku.trim().toLowerCase());
      ...
      saveDb(KEYS.PRODUCTS, products);
      addMockAuditLog('PRODUCT_UPDATED', 'Product', id, `Cập nhật SKU: ${updated.sku} (${updated.name})`);
      return updated;
    }
  ...
  createWarehouse: async (whData) => {
    if (useMock) {
      ...
      const codeExists = warehouses.some(w => w.code.trim().toLowerCase() === whData.code.trim().toLowerCase());
      ...
      addMockAuditLog('WAREHOUSE_CREATED', 'Warehouse', newWh.id, `Tạo mới kho: ${newWh.name} (${newWh.code})`);
      return newWh;
    }
    const response = await apiClient.post('/warehouses', whData);
    return response.data;
  },
  ...
  updateWarehouse: async (id, whData) => {
    if (useMock) {
      ...
      const codeExists = warehouses.some(w => w.id !== Number(id) && w.code.trim().toLowerCase() === whData.code.trim().toLowerCase());
      if (codeExists) throw new Error('DUPLICATE_WAREHOUSE_CODE');
      ...
      saveDb(KEYS.WAREHOUSES, warehouses);
      addMockAuditLog('WAREHOUSE_UPDATED', 'Warehouse', id, `Cập nhật kho: ${updated.name} (${updated.code})`);
      return updated;
    }
    const response = await apiClient.put(`/warehouses/${id}`, whData);
    return response.data;
  },
  ...
  toggleWarehouseStatus: async (id, isActive) => {
    if (useMock) {
      ...
      const actionName = isActive ? 'WAREHOUSE_ACTIVATED' : 'WAREHOUSE_DEACTIVATED';
      addMockAuditLog(actionName, 'Warehouse', id, `${isActive ? 'Kích hoạt' : 'Khóa'} kho: ${warehouses[idx].code}`);
      return warehouses[idx];
    }
    const response = await apiClient.put(`/warehouses/${id}/status`, { is_active: isActive });
    return response.data;
  },
  ...
  getBinLocations: async (warehouseId) => {
    ...
    const url = warehouseId ? `/bin-locations?warehouseId=${warehouseId}` : '/bin-locations';
  ...
  createBinLocation: async (binData) => {
    if (useMock) {
      ...
      const newBin = {
        id: locations.length > 0 ? Math.max(...locations.map(l => l.id)) + 1 : 101,
      ...
      addMockAuditLog('BIN_CREATED', 'BinLocation', newBin.id, `Tạo mới vị trí Bin: ${newBin.code} tại kho ${wh.code}`);
      return newBin;
    }
    const response = await apiClient.post('/bin-locations', binData);
    return response.data;
  },
  ...
  updateBinLocation: async (id, binData) => {
    if (useMock) {
      ...
      const updatedBin = {
        ...
        is_quarantine: binData.is_quarantine !== undefined ? !!binData.is_quarantine : oldBin.is_quarantine
      };

      locations[idx] = updatedBin;
      saveDb(KEYS.LOCATIONS, locations);
      addMockAuditLog('BIN_UPDATED', 'BinLocation', id, `Cập nhật cấu hình Bin: ${updatedBin.code}`);
      return updatedBin;
    }
    const response = await apiClient.put(`/bin-locations/${id}`, binData);
    return response.data;
  },
  ...
  toggleBinStatus: async (id, isActive) => {
    if (useMock) {
      ...
      addMockAuditLog(actionName, 'BinLocation', id, `${isActive ? 'Kích hoạt' : 'Khóa'} vị trí Bin: ${locations[idx].code}`);
      return locations[idx];
    }
    const response = await apiClient.put(`/bin-locations/${id}/status`, { is_active: isActive });
    return response.data;
  },
  ...
  createDealer: async (dlData) => {
    if (useMock) {
      ...
      const exists = dealers.some(d => d.code.trim().toLowerCase() === dlData.code.trim().toLowerCase());
      ...
      addMockAuditLog('DEALER_CREATED', 'Dealer', newDl.id, `Tạo mới đại lý: ${newDl.name} (${newDl.code})`);
      return newDl;
    }
  ...
  updateDealer: async (id, dlData) => {
    if (useMock) {
      ...
      dealers[idx] = updated;
      saveDb(KEYS.DEALERS, dealers);
      addMockAuditLog('DEALER_UPDATED', 'Dealer', id, `Cập nhật thông tin đại lý: ${updated.name} (${updated.code})`);
      return updated;
    }
  ...
  updateDealerCreditLimit: async (id, creditLimitData) => {
    ...
    const response = await apiClient.put(`/dealers/${id}/credit-limit`, creditLimitData);
    return response.data;
  },
  ...
  toggleDealerStatus: async (id, isActive) => {
    if (useMock) {
      ...
      addMockAuditLog(actionName, 'Dealer', id, `${isActive ? 'Kích hoạt' : 'Khóa'} đối tác Đại lý: ${dealers[idx].code}`);
      return dealers[idx];
    }
    const response = await apiClient.put(`/dealers/${id}/status`, { is_active: isActive });
    return response.data;
  },
  ...
  createSupplier: async (splData) => {
    if (useMock) {
      ...
      const exists = suppliers.some(s => s.code.trim().toLowerCase() === splData.code.trim().toLowerCase());
      ...
      addMockAuditLog('SUPPLIER_CREATED', 'Supplier', newSpl.id, `Tạo mới nhà cung cấp: ${newSpl.company_name} (${newSpl.code})`);
      return newSpl;
    }
  ...
  updateSupplier: async (id, splData) => {
    if (useMock) {
      ...
      saveDb(KEYS.SUPPLIERS, suppliers);
      addMockAuditLog('SUPPLIER_UPDATED', 'Supplier', id, `Cập nhật thông tin nhà cung cấp: ${updated.company_name} (${updated.code})`);
      return updated;
    }
  ...
  toggleSupplierStatus: async (id, isActive) => {
    if (useMock) {
      ...
      const actionName = isActive ? 'SUPPLIER_ACTIVATED' : 'SUPPLIER_DEACTIVATED';
      addMockAuditLog(actionName, 'Supplier', id, `${isActive ? 'Kích hoạt' : 'Khóa'} đối tác Nhà CC: ${suppliers[idx].code}`);
      return suppliers[idx];
    }
    const response = await apiClient.put(`/suppliers/${id}/status`, { is_active: isActive });
    return response.data;
  },
  ...
  createVehicle: async (vhData) => {
    if (useMock) {
      ...
      const exists = vehicles.some(v => v.plate_number.trim().toLowerCase() === vhData.plate_number.trim().toLowerCase());
      ...
      addMockAuditLog('VEHICLE_CREATED', 'Vehicle', newVh.id, `Thêm phương tiện mới: ${newVh.plate_number}`);
      return newVh;
    }
    const response = await apiClient.post('/vehicles', vhData);
    return response.data;
  },
  ...
  updateVehicle: async (id, vhData) => {
    if (useMock) {
      ...
      const plateExists = vehicles.some(v => v.id !== Number(id) && v.plate_number.trim().toLowerCase() === vhData.plate_number.trim().toLowerCase());
      ...
      const updated = {
        ...
        plate_number: vhData.plate_number.trim().toUpperCase(),
        vehicle_type: vhData.vehicle_type.trim(),
        max_weight_kg: parseFloat(vhData.max_weight_kg) || vehicles[idx].max_weight_kg,
        max_volume_m3: parseFloat(vhData.max_volume_m3) || vehicles[idx].max_volume_m3,
        status: vhData.status || vehicles[idx].status
      };

      vehicles[idx] = updated;
      saveDb(KEYS.VEHICLES, vehicles);
      addMockAuditLog('VEHICLE_UPDATED', 'Vehicle', id, `Cập nhật phương tiện: ${updated.plate_number} (Trạng thái: ${updated.status})`);
      return updated;
    }
    const response = await apiClient.put(`/vehicles/${id}`, vhData);
    return response.data;
  },
  ...
  toggleVehicleStatus: async (id, isActive) => {
    if (useMock) {
      ...
      addMockAuditLog(actionName, 'Vehicle', id, `${isActive ? 'Kích hoạt' : 'Khóa'} phương tiện: ${vehicles[idx].plate_number}`);
      return vehicles[idx];
    }
    const response = await apiClient.put(`/vehicles/${id}/status`, { is_active: isActive });
    return response.data;
  },
  ...
  createDriver: async (drvData) => {
    if (useMock) {
      ...
      const lExists = drivers.some(d => d.license_number.trim().toLowerCase() === drvData.license_number.trim().toLowerCase());
      ...
      addMockAuditLog('DRIVER_CREATED', 'Driver', newDrv.id, `Đăng ký tài xế mới: ${newDrv.full_name} (${newDrv.license_number})`);
      return newDrv;
    }
    const response = await apiClient.post('/drivers', drvData);
    return response.data;
  },
  ...
  updateDriver: async (id, drvData) => {
    if (useMock) {
      ...
      const lExists = drivers.some(d => d.id !== Number(id) && d.license_number.trim().toLowerCase() === drvData.license_number.trim().toLowerCase());
      if (lExists) throw new Error('DUPLICATE_LICENSE_NUMBER');
      ...
      drivers[idx] = updated;
      saveDb(KEYS.DRIVERS, drivers);
      addMockAuditLog('DRIVER_UPDATED', 'Driver', id, `Cập nhật tài xế: ${updated.full_name} (Trạng thái: ${updated.status})`);
      return updated;
    }
    const response = await apiClient.put(`/drivers/${id}`, drvData);
    return response.data;
  },
  ...
  toggleDriverStatus: async (id, isActive) => {
    if (useMock) {
      ...
      addMockAuditLog(actionName, 'Driver', id, `${isActive ? 'Kích hoạt' : 'Khóa'} tài xế: ${drivers[idx].full_name}`);
      return drivers[idx];
    }
    const response = await apiClient.put(`/drivers/${id}/status`, { is_active: isActive });
    return response.data;
  }
};
