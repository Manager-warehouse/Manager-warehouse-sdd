      ...
      >
          ...
          <div>
            ...
            <nav className="flex flex-col gap-1">
              {menuItems
                .filter(
                  (item) =>
                    !(item.hiddenForRoles || []).some((role) => hasRole(role)),
                )
                .filter(
                  (item) =>
                    item.roles.length === 0 ||
        ...
        <div className="p-4 border-t border-hairline-dark flex flex-col gap-1.5 bg-canvas-nightElevated">
          ...
          <div className="flex flex-wrap gap-1">
            {user?.role && (
              <span className="text-[9px] font-semibold bg-shade-70 text-onPrimary px-2 py-0.5 rounded-pill border border-shade-60">
