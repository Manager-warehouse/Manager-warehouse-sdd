...
const exportableAllocations = (item) => {
  ...
  return allocations
    .map((allocation) => {
      if (!allocation.qc_completed) return allocation;

      const passQty = Number(allocation.qc_pass_qty || 0);
      if (passQty <= 0) return null;
...
const PickingListTable = ({ items = [] }) => {
  const rows = items.flatMap((item) =>
    exportableAllocations(item).map((allocation, index) => ({ item, allocation, index })),
  );
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-canvas-cream border-b border-hairline-light">
              ...
              <th className="px-6 py-3.5 text-xs font-bold text-shade-60 uppercase tracking-wider text-right">SL xuất kho</th>
            </tr>
          </thead>

          <tbody className="divide-y divide-hairline-light">
            {rows.map(({ item, allocation, index }) => (
            ...
            {!rows.length && (
              <tr>
                <td colSpan="5" className="px-6 py-10 text-center text-sm text-shade-40 italic">
                  Đơn này chưa có dòng hàng xuất kho nào.
                </td>
              </tr>
            )}
          </tbody>
        ...
        {rows.map(({ item, allocation, index }) => (
          ...
          >
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
              ...
              <div className="rounded-md border border-hairline-light p-2">
                <span className="block text-[10px] uppercase tracking-wider text-shade-50">Xuất kho</span>
