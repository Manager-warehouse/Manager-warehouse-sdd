...
1. **`auth.store.js`** ([auth.store.js](file:///d:/swp/Manager-warehouse-sdd/frontend/src/stores/auth.store.js)):
   ...
   * **Actions**:
     * `login(user, token)`: Lưu token, user vào `sessionStorage` (phiên làm việc bị xóa khi đóng tab, an toàn hơn `localStorage`), tính toán và thiết lập `activeWarehouse` mặc định (kho đầu tiên được gán, hoặc HP-01 đối với Admin/CEO).
     * `logout()`: Xóa sạch session trong `sessionStorage` và reset state.
...
* **Interceptors**:
  * **Request Interceptor**: Tự động gắn header `Authorization: Bearer <token>` nếu có token trong `sessionStorage`.
