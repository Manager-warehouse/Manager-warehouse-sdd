...
def update_round3_excel(results_dict: dict):
    """Same shape/contract as update_round2_excel, but targets the Round 3
    columns (L/M/N Status/Date/Tester, row 8 aggregate per
    docs/test/fix_all_statistics.py's row mapping: row 6=Round1, 7=Round2,
    8=Round3)."""
    if not EXCEL_PATH.exists():
        print(f"Excel file not found: {EXCEL_PATH}")
        return
    ...
    for sheet_name, tc_list in results_dict.items():
        ...
        for tc in tc_list:
            ...
            if tc_id in tc_row_map:
                ...
                # Round 3 Tester (Col 14 / N)
                c14 = ws.cell(row=r, column=14)
                c14.value = TESTER_NAME
                c14.font = tahoma_regular
                ...
                if note:
                    c15 = ws.cell(row=r, column=15)
                    old_note = str(c15.value or '')
                    c15.value = f"{old_note} | Selenium R3: {note}".strip(" |")
                    c15.font = tahoma_regular
                    c15.alignment = align_left

        # Update Round 3 formula counts in Module sheet header row 8
    ...
    wb.save(str(EXCEL_PATH))
    print(f"[Excel Report] Successfully updated Round 3 in {EXCEL_PATH}")
    print(f"  -> Round 3 Selenium Passed: {total_r3_passed}, Failed: {total_r3_failed}")
    return total_r3_passed, total_r3_failed


def update_round3_markdown(results_dict: dict):
    if not MD_PATH.exists():
        return
    ...
    summary_section = [
        ...
        "---",
        "",
        f"## 4. KẾT QUẢ KHIỂM THỬ SELENIUM E2E (ROUND 3 SYSTEM TEST -- REAL BUSINESS FLOWS)",
        f"**Ngày chạy Selenium:** {RUN_DATE}",
        f"**Công cụ thực thi:** Selenium WebDriver (Python)",
        f"**Người thực hiện / Script:** {TESTER_NAME}",
        "",
        "| Module Code | Tên Module | Selenium Tested | Passed | Failed | Status |",
        "|---|---|---|---|---|---|",
