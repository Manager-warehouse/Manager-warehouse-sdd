...
| 05  | `products`                        | Danh mục SKU đồ gia dụng — không serial, không hạn dùng, không grade; legacy serial/expiry columns removed by V17.                                                                                                          |
| 06  | `price_history`                   | Giá vốn/giá bán theo `effective_date`; không có `end_date` từ V6.                                                                                                                                                           |
| 07  | `batches`                         | Lô hàng theo `product_id` + `warehouse_id` + `received_date` — khóa FIFO; không có expiry/grade.                                                                                                                        |
