...
- Q: Lập hóa đơn bán hàng (US-WMS-10) trước đây được mô tả là tự động ngay khi giao hàng OTP+POD hợp lệ. Vì sao đổi sang thủ công, và mục 10 (Out of Scope) từng nói `POST /api/v1/invoices` "không có giao diện thao tác" có còn đúng không? → A: Không còn đúng — đã sửa. Lập hóa đơn bán hàng giờ là thao tác thủ công của Kế toán viên qua worklist `billing_notifications`, đối xứng có chủ đích với luồng AP (US-WMS-28) vốn luôn thủ công (hóa đơn mua hàng cần số hóa đơn VAT từ NCC, một dữ liệu bên ngoài không thể tự sinh). Lý do đầy đủ và rủi ro chuyển đổi (DO `COMPLETED` từ trước khi có cơ chế này sẽ không có `billing_notifications`) nằm ở `feature-accountant-customer-invoicing.md` §1. `DriverDeliveryServiceImpl.confirmDelivery()` giờ chỉ tạo `billing_notifications`, không còn gọi `AutoInvoiceService` trực tiếp; `POST /api/v1/invoices` (`InvoiceServiceImpl` → `AutoInvoiceService.createBackfillInvoice`) là luồng tạo hóa đơn chính, có giao diện thao tác đầy đủ ("Lập Hóa đơn Bán" trên tab thông báo AR).
- Q: `dealers.current_balance` có được phép âm không? Kịch bản nào khiến nó xảy ra? → A: Có, hệ thống chấp nhận. `ReturnsService.createCreditNote()` trừ thẳng `totalRefundAmount` (tính từ `receiptItem.actualQty × unitCost` của phiếu trả hàng, không phải từ dư nợ hóa đơn gốc) vào `dealer.currentBalance` mà không kiểm tra sàn 0 — nếu một đại lý trả lại hàng trị giá lớn hơn dư nợ hiện có (ví dụ đã thanh toán gần hết hóa đơn trước khi trả hàng, hoặc nhiều Credit Note dồn lại), `current_balance` có thể xuống âm, nghĩa là hệ thống giờ nợ ngược lại đại lý. Đây là quyết định chấp nhận có ý thức trong phạm vi Sprint 1, không phải lỗi: xử lý số dư âm (hoàn tiền mặt, hay cấn trừ vào đơn hàng kế tiếp) nằm ngoài phạm vi Spec 008/009 hiện tại và chưa có cơ chế hoàn tiền tự động.
- Q: Báo cáo Công nợ Phân kỳ (US-WMS-16) hiển thị số dư âm này như thế nào? → A: Có một điểm không nhất quán cần lưu ý khi đọc báo cáo: trường `current_balance` trong response trả thẳng giá trị đã lưu của `dealers.current_balance` (có thể âm), nhưng các cột phân kỳ (`in_term_amount`, `overdue_1_to_30`...) được tính bằng cách cộng dồn `creditNotePool` (tổng toàn bộ Credit Note của đại lý) trừ dần vào từng hóa đơn chưa thanh toán theo `due_date` cũ nhất trước (xem `PaymentReceiptServiceImpl.getCreditAgingReport()`), và mỗi hóa đơn được chặn sàn ở 0 (`effectiveDebt` không bao giờ âm) — phần Credit Note dư ra sau khi đã trừ hết mọi hóa đơn chưa thanh toán bị bỏ qua, không xuất hiện ở đâu trong các cột phân kỳ. Kết quả: nếu `current_balance` âm, tổng 5 cột phân kỳ (`in_term_amount + overdue_1_to_30 + ... `) sẽ bằng 0, không bằng `current_balance` (âm) — đây là hành vi đã biết, không phải lỗi tính toán, nhưng người đọc báo cáo cần hiểu `current_balance` là "sự thật" còn các cột phân kỳ chỉ mô tả nợ-chưa-thu (luôn ≥ 0), không mô tả phần dư đại lý được hoàn lại.
...
- `current_balance` (DECIMAL(18,2), NOT NULL, DEFAULT 0) - Dư nợ hiện tại (dương = đại lý còn nợ hệ thống). **Có thể âm** — xem Session 2026-07-25 (Clarifications) về cách `credit_notes` (Spec 009) trừ trực tiếp vào cột này mà không kiểm tra sàn 0.
...
- `created_by` (BIGINT, FK→users, NOT NULL)
- `document_date` (DATE, NOT NULL) - Ngày Nhập Hàng
- `accounting_period_id` (BIGINT, FK→accounting_periods, NOT NULL) - Kỳ kế toán phát sinh
- `notes` (TEXT)
- `created_at` (TIMESTAMPTZ, DEFAULT NOW())
...
| `PERIOD_CLOSED`                   | 422 Unprocessable Entity | Kỳ kế toán của Ngày Nhập Hàng đã bị đóng, không cho phép ghi nhận   |
