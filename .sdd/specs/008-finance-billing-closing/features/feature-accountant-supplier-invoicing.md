...
## 1. Context and Goal
Khi phiếu nhập kho (`receipt_orders`) được hoàn tất kiểm đếm, QC và duyệt xếp hàng vào bin thành công (chuyển trạng thái sang `COMPLETED` ở Spec 003), hệ thống **tự động** tạo thông báo lập hóa đơn mua hàng (`supplier_billing_notifications`). 
...
## 2. Actors
* **Hệ thống**: Tự động sinh thông báo `supplier_billing_notifications` khi phiếu nhập kho (`receipt_orders`) chuyển sang trạng thái `COMPLETED`.
...
* **Event-driven:**
  * **WHEN** phiếu nhập kho (`receipt_orders`) chuyển sang trạng thái `COMPLETED` theo nghiệp vụ tại Spec 003, hệ thống **SHALL** trong cùng giao dịch:
  ...
  * **WHEN** Kế toán viên gửi yêu cầu lập hóa đơn mua hàng qua `POST /api/v1/supplier-invoices` với `receiptOrderId` hợp lệ, hệ thống **SHALL**:
    * Kiểm tra phiếu nhập kho `receipt_orders` phải ở trạng thái `COMPLETED` và chưa từng lập hóa đơn mua hàng trước đó (`supplier_invoices.receipt_order_id` chưa tồn tại).
...
* **State-driven:**
  * **WHILE** phiếu nhập kho gốc chưa ở trạng thái `COMPLETED`, hệ thống **SHALL** từ chối lập hóa đơn mua hàng (trả về lỗi `RECEIPT_ORDER_NOT_COMPLETED` với HTTP 400).
  ...
  [
    {
      "id": 1,
      "receiptOrderId": 10,
      "receiptOrderNumber": "RO-20260710-001",
      "supplierId": 5,
...
* **Scenario: Lập Hóa đơn Mua hàng ghi nhận nợ NCC thành công**
  * **Given**: Phiếu nhập kho `RO-20260710-001` đã ở trạng thái `COMPLETED` với tổng giá trị `45,000,000` VNĐ, dư nợ hiện tại của NCC là `0` VNĐ.
...
* **Scenario: Chặn lập hóa đơn cho phiếu nhập chưa COMPLETED**
  ...
  * **Then**: Hệ thống từ chối yêu cầu và trả về lỗi `RECEIPT_ORDER_NOT_COMPLETED` (HTTP 400).
