...
- **State-driven:**
  - **WHILE** một kỳ kế toán có trạng thái `status = 'CLOSED'`, hệ thống **SHALL** chặn tất cả các yêu cầu tạo mới (CREATE), chỉnh sửa (UPDATE) hoặc xóa (DELETE) đối với mọi chứng từ (bao gồm `receipts`, `delivery_orders`, `transfers`, `stock_takes`, `adjustments`, `invoices`, `payment_receipts`, `supplier_invoices`, `supplier_payments`, `debit_notes`) có Ngày Nhập Hàng `document_date` thuộc phạm vi từ `start_date` đến `end_date` của kỳ đã đóng đó (trả về lỗi `PERIOD_CLOSED` với HTTP 422). Danh sách này khớp với các loại `referenceType` mà [Correction Voucher](./feature-accountant-correction-voucher.md) chấp nhận — mọi chứng từ tài chính không có endpoint UPDATE/DELETE đều phải được khóa cứng theo kỳ như nhau, không riêng `invoices`/`payment_receipts`.
