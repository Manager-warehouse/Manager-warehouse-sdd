...
* **Event-driven:**
  ...
  * **WHEN** một chứng từ nghiệp vụ được tạo với `document_date` thuộc một kỳ kế toán đã `CLOSED`, hệ thống **SHALL** từ chối `document_date` gốc và hạch toán chứng từ vào kỳ kế toán hiện đang `OPEN` thay vì kỳ đã đóng (không lùi ngày vào kỳ đã khóa).
  * **WHEN** phát hiện sai sót thuộc một kỳ đã `CLOSED` sau khi kỳ đó đã khóa, hệ thống **SHALL** yêu cầu tạo một bản ghi `adjustments` với `type = 'CORRECTION_VOUCHER'`, `document_date` thuộc kỳ hiện đang `OPEN`, và `reference_id`/`reference_type` trỏ về chứng từ gốc cần điều chỉnh — hệ thống **SHALL NOT** cho phép mở lại hoặc sửa trực tiếp chứng từ trong kỳ đã đóng (safety.md DAT-05).
* **State-driven:**
