...
**Ràng buộc danh sách tài xế dùng chung**: Danh sách chuyến điều chuyển trong module tài xế chỉ được đọc và không được mutate transfer status, trip status, inventory, resource assignment hoặc audit logs. Danh sách dùng chung có thể lọc cục bộ bằng `tripType`; khi mở trip `TRANSFER` vẫn phải dùng action của Spec 005, không dùng action giao đại lý của Spec 004. Riêng list/detail của module điều chuyển có thể normalize transfer quá hạn theo rule cancel trước `IN_TRANSIT` hoặc quay đầu khi đã `IN_TRANSIT`.
...
├── controller/warehouse_transfer/InterWarehouseTransferController.java
├── controller/warehouse_transfer/TransferRequestController.java
...
├── entity/warehouse_transfer/InterWarehouseTransfer.java
├── entity/warehouse_transfer/InterWarehouseTransferItem.java
├── entity/warehouse_transfer/InterWarehouseTransferAllocation.java
├── entity/warehouse_transfer/TransferRequest.java
├── entity/warehouse_transfer/TransferRequestItem.java
├── enums/warehouse_transfer/InterWarehouseTransferStatus.java
├── enums/warehouse_transfer/TransferRequestStatus.java
├── enums/AuditAction.java
├── repository/warehouse_transfer/InterWarehouseTransferRepository.java
├── repository/warehouse_transfer/InterWarehouseTransferItemRepository.java
├── repository/warehouse_transfer/InterWarehouseTransferAllocationRepository.java
├── repository/warehouse_transfer/TransferRequestRepository.java
├── mapper/InterWarehouseTransferMapper.java
├── service/warehouse_transfer/InterWarehouseTransferService.java
├── service/warehouse_transfer/TransferRequestService.java
├── service/warehouse_transfer/impl/InterWarehouseTransferServiceImpl.java
├── service/warehouse_transfer/impl/InterWarehouseTransferPlanningService.java
├── service/warehouse_transfer/impl/InterWarehouseTransferApprovalService.java
├── service/warehouse_transfer/impl/InterWarehouseTransferShippingService.java
├── service/warehouse_transfer/impl/InterWarehouseTransferReceivingService.java
├── service/warehouse_transfer/impl/InterWarehouseTransferHelper.java
└── service/warehouse_transfer/impl/TransferRequestServiceImpl.java
...
1. Thêm Flyway migration cộng thêm sau migration mới nhất đã deploy để đồng bộ constraint status điều chuyển, cho planned item `batch_id` nullable, thêm version columns, timestamp arrival/handover, outbound QC fields, trọng lượng/thể tích tính toán của trip và dữ liệu discrepancy incident/hold. Không sửa `V1`-`V5`.
...
7. Gỡ nhánh wrong-SKU return khỏi runtime; sai SKU nếu phát sinh được xử lý qua count/QC/discrepancy/quarantine theo trạng thái vật lý.
8. Giới hạn overdue return-to-source cho transfer `IN_TRANSIT` thật sự quá hạn; trước `IN_TRANSIT` thì tự hủy/cancel để trả reservation.
