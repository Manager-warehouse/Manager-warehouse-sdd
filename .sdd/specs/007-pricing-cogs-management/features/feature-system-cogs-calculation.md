...
- The system SHALL always look up `price_history` WHERE `status = 'APPROVED' AND effective_date <= :today AND end_date >= :today` for each product line when a DO is created.
...
- WHEN Planner creates a DO with one or more product lines:
  - For each `delivery_order_item`, query: `SELECT * FROM price_history WHERE product_id = ? AND status = 'APPROVED' AND effective_date <= CURRENT_DATE AND end_date >= CURRENT_DATE`.
...
- WHILE no `APPROVED` price entry exists for a product on `CURRENT_DATE`, any DO line containing that product SHALL be blocked.
...
`GET /api/v1/price-history/lookup?product_id={id}&date={YYYY-MM-DD}`

- Trả về bản giá `APPROVED` có hiệu lực tại ngày `date`, hoặc HTTP 404 nếu không có.
...
- Given product P có APPROVED: effective 01/06, end 30/06, cost 80.000, sell 115.000
- When Planner tạo DO chứa P ngày 15/06
- Then `delivery_order_items.unit_price = 115.000`, `unit_cost = 80.000`
- And DO được tạo thành công

**Scenario 2: DO bị chặn vì thiếu giá**
- Given product P không có APPROVED nào có hiệu lực ngày 15/07
...
- Given DO có 3 dòng: product A và B có giá hợp lệ, product C không có giá
- When Planner tạo DO
...
- When GET /price-history/lookup?product_id=P&date=2026-06-15
- Then trả về bản giá với cost/selling price tương ứng
- When GET với date=2026-07-01 (không có giá tháng 7)
- Then HTTP 404
