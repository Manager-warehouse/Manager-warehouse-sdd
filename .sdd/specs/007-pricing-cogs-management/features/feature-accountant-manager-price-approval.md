...
- For each PENDING entry, the system SHALL display, for comparison, the `APPROVED` entry for the same `(product_id, warehouse_id)` with the largest `effective_date` that is still `<=` the PENDING entry's own `effective_date` — i.e. the entry it will actually supersede once approved — or indicate "Chưa có giá hiệu lực tại kho này" if none exists. This is **not** simply "the most recent `APPROVED` entry overall" (Session 2026-07-18 của `spec.md`): those two are the same in the common case (creating a price for today/the future), but diverge when Kế toán viên backdates an entry to fill in a past window while a newer `APPROVED` entry already exists — comparing against "most recent overall" in that case would compare against a price that comes chronologically *after* the one being reviewed, which is misleading.
...
- WHILE viewing the approval list, the system SHALL show the delta between `cost_price`/`selling_price` of the PENDING entry and its comparison baseline as defined in 3.2 above (absolute difference and percentage).
...
| `GET` | `/api/v1/price-history/{id}` | Chi tiết một bản giá; response luôn bổ sung `previous_approved` cho bất kỳ caller nào có quyền xem bản giá đó (không riêng `ACCOUNTANT_MANAGER` — Session 2026-07-18 của `spec.md`) — endpoint dùng chung, owned bởi feature 14a |
...
**Scenario 4b: Delta so với bản giá backdated so sánh đúng bản bị thay thế, không phải bản mới nhất**
- Given product P tại kho Hải Phòng có hai bản APPROVED: effective 01/06 (cost 80.000, sell 115.000) và effective 01/08 (cost 90.000, sell 130.000)
- And Kế toán viên tạo bản PENDING mới effective 01/07 (bù trừ cho một giai đoạn giữa hai mốc trên), cost 82.000, sell 118.000
- When GET /price-history/{id-của-bản-PENDING-01/07}
- Then `previous_approved` là bản effective 01/06 (bản mà bản 01/07 sẽ thay thế khi duyệt), KHÔNG phải bản effective 01/08
- And `cost_price_delta = 2000` (82.000 − 80.000), không phải so với bản 01/08
