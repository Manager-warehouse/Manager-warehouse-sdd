...
- The system SHALL re-validate APPROVED overlap at the moment of approval, not only at creation time. If a concurrent approval has occurred for the same `(product_id, warehouse_id)` since the entry was created, the system SHALL reject with `OVERLAPPING_EFFECTIVE_DATE` (409).
...
- WHEN Kế toán trưởng submits `PUT /api/v1/price-history/{id}/approve`:
  ...
  - Re-check APPROVED overlap for the same `(product_id, warehouse_id)` and date range. If overlap found → HTTP 409 `OVERLAPPING_EFFECTIVE_DATE`.
...
- Given bản giá ID=15 (tháng 7, Hải Phòng) ở PENDING
- And trong thời gian chờ, bản giá ID=16 cùng product cùng kho Hải Phòng cùng tháng 7 đã được APPROVED trước
