...
- The system SHALL re-validate for a conflicting `APPROVED` entry at the moment of approval, not only at creation time. If a concurrent approval has created another `APPROVED` entry with the same `(product_id, warehouse_id, effective_date)` since this entry was created, the system SHALL reject with `OVERLAPPING_EFFECTIVE_DATE` (409). There is no `end_date`; entries with different `effective_date` never conflict — the later one simply supersedes the earlier one for lookups from that date forward.
...
- WHEN Kế toán trưởng submits `PUT /api/v1/price-history/{id}/approve`:
  ...
  - Re-check for another `APPROVED` entry with the same `(product_id, warehouse_id, effective_date)`. If found → HTTP 409 `OVERLAPPING_EFFECTIVE_DATE`.
...
**Scenario 2: Từ chối vì trùng effective_date phát sinh sau khi tạo (race condition)**
- Given bản giá ID=15 (product P, kho Hải Phòng, effective 01/07) ở PENDING
- And trong thời gian chờ, bản giá ID=16 cùng product, cùng kho Hải Phòng, cùng effective_date 01/07 đã được APPROVED trước
