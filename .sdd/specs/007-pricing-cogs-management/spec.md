...
**Q: Phân quyền Quản lý Bảng giá & Phê duyệt Giá (`007-pricing-cogs-management`)?**
A: Tuân thủ phân quyền theo `role.md`: `ACCOUNTANT` (Kế toán viên) nhập và trình duyệt bảng giá; `ACCT_MANAGER` (Kế toán trưởng) độc quyền phê duyệt bảng giá (`Price Approval`). Đảm bảo nguyên tắc Maker-Checker (`ACCOUNTANT` không được tự duyệt giá của chính mình, `CEO` không duyệt giá trực tiếp trên màn hình Price Approval).
