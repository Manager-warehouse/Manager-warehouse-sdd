...
A: Đây là một **ngoại lệ nghiệp vụ bắt buộc**. Để bảo vệ dữ liệu tài chính, giá vốn, lãi lỗ và công nợ nhạy cảm của doanh nghiệp, mọi thao tác truy cập CEO Dashboard hoặc xuất báo cáo tài chính/năng suất đều phải được ghi audit log với `action = 'VIEW'`, `entity_type = 'REPORT'`, `entity_id = 0`, và thông tin bộ lọc/loại báo cáo được lưu trong cột `new_value`. Các màn hình danh sách thông thường khác của hệ thống vẫn tuân thủ không ghi log read-only.
...
| **Trưởng kho** | Operations Checker | Xem cảnh báo tồn kho thấp của kho được gán phụ trách; Xem và xuất báo cáo năng suất nhân viên thuộc kho của mình quản lý. |
| **Planner** | Planning Maker | Xem cảnh báo tồn kho thấp của tất cả các kho để điều phối hàng hóa hoặc lập kế hoạch tái nhập. |
| **System** | Background Trigger | Tự động giám sát biến động tồn kho để kích hoạt/giải quyết cảnh báo; tự động ghi audit log khi xem báo cáo. |
...
| **NFR-004** | Thời gian xuất báo cáo năng suất dạng Excel (1.000 dòng) | ≤ 3.0 giây |
| **NFR-005** | Khả năng chịu tải đồng thời khi truy vấn báo cáo | Hỗ trợ tối thiểu 20 truy vấn báo cáo đồng thời không giảm hiệu năng |
...
### audit_logs (Ngoại lệ xem Báo cáo)

Mỗi lần API báo cáo được gọi thành công, hệ thống tự động chèn một bản ghi mới vào bảng `audit_logs` chung của hệ thống (đã định nghĩa trong Spec 001) với quy tắc điền giá trị như sau:

| Field | Value / Rule |
|-------|--------------|
| `actor_id` | ID của người dùng thực hiện yêu cầu |
| `actor_role` | Role của người dùng tại thời điểm yêu cầu (`CEO`, `ACCOUNTANT_MANAGER`, `WAREHOUSE_MANAGER`...) |
| `action` | `'VIEW'` |
| `entity_type` | `'REPORT'` |
| `entity_id` | `0` |
| `description` | Ví dụ: `"Truy cập CEO Dashboard"` hoặc `"Xuất báo cáo năng suất kho Hà Nội"` |
| `warehouse_id` | ID kho trong bộ lọc yêu cầu (nullable nếu là báo cáo tổng hợp toàn quốc) |
| `old_value` | `NULL` |
| `new_value` | JSONB chứa thông tin loại báo cáo và các tham số lọc, ví dụ: `{"report_type": "CEO_DASHBOARD", "filters": {"period": "2026-06"}}` |
| `timestamp` | `NOW()` |
...
| `WAREHOUSE_SCOPE_FORBIDDEN` | 403 | Trưởng kho yêu cầu xem cảnh báo hoặc báo cáo năng suất của kho mà mình không được gán quyền phụ trách. |
...
| `EXPORT_FAILED` | 500 | Lỗi hệ thống trong quá trình biên dịch và xuất file Excel. |
