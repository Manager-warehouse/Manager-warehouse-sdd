...
### Session 2026-07-23
- Q: Phân quyền truy cập CEO Dashboard (`GET /api/v1/dashboard/ceo`) → A: Giới hạn CEO Dashboard chỉ dành riêng cho `CEO` (Đúng chuẩn `role.md`). `ACCT_MANAGER` xem Báo cáo giá trị tồn kho (`/api/v1/reports/inventory-valuation`).

---

## 2. Actor

- **CEO (`CEO`)** — Viewer cấp cao: Xem toàn bộ các chỉ số trên CEO Dashboard quản trị.
- **Kế toán trưởng (`ACCT_MANAGER`)** — Viewer tài chính: Xem/xuất báo cáo Giá trị tồn kho và các báo cáo công nợ/tài chính liên quan.
...
- The system SHALL restrict access to CEO Dashboard APIs (`GET /api/v1/dashboard/ceo`) strictly to users with role `CEO`; unauthorized access requests (including non-CEO roles) SHALL be rejected with `ACCESS_DENIED` (403).
...
- WHEN a user with `CEO` role requests `GET /api/v1/dashboard/ceo`:
