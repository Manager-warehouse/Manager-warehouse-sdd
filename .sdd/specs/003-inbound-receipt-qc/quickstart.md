...
- QC all passed -> `QC_COMPLETED`.
- QC has failed quantity -> `QC_FAILED`; failed quantity is staged as Quarantine readiness only.
...
- Putaway from `APPROVED` or `PARTIALLY_APPROVED` increases regular inventory by `approved_qty` only.
- Whole-receipt rejection requires `WH_MANAGER` role, warehouse scope, version, and reason.
...
3. WH_STAFF or STOREKEEPER submits complete count -> `DRAFT`.
4. WH_STAFF, STOREKEEPER, or WH_MANAGER confirms all goods passed QC -> `QC_COMPLETED`.
5. WH_MANAGER approves -> `APPROVED`; available regular stock is unchanged.
6. STOREKEEPER putaway into regular bin -> `PUTAWAY_COMPLETED`; available regular stock increases.

## Manual Flow 2: Some Goods Failed QC

1. WH_STAFF or STOREKEEPER submits count -> `DRAFT`.
2. WH_STAFF, STOREKEEPER, or WH_MANAGER confirms QC with passed and failed quantities -> `QC_FAILED`.
...
5. STOREKEEPER putaway approved passed quantity -> `PUTAWAY_COMPLETED`.
...
## Manual Flow 3: Correction And Cancel

1. Before WH_MANAGER finalization, WH_STAFF or STOREKEEPER corrects counts after QC -> QC fields and Quarantine readiness are cleared, receipt returns to `DRAFT`.
...
## Manual Flow 3A: Pre-Receive Rejected For Correction

1. PLANNER creates purchase receipt -> `PENDING_MANAGER_APPROVAL`.
2. WH_MANAGER rejects with reason -> `REVISION_REQUIRED`.
3. WH_STAFF or STOREKEEPER count attempt returns `RECEIPT_PENDING_MANAGER_APPROVAL`.
4. PLANNER corrects and resubmits -> `PENDING_MANAGER_APPROVAL`.
5. WH_MANAGER approves -> `PENDING_RECEIPT`.

## Manual Flow 4: Whole Receipt Rejected

1. Receipt is `QC_COMPLETED` or `QC_FAILED`.
2. WH_MANAGER rejects with reason -> `RETURN_TO_SUPPLIER_PENDING`.
3. STOREKEEPER confirms supplier handover -> `RETURNED_TO_SUPPLIER`.
