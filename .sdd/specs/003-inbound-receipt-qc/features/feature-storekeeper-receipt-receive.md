...
* **Event-driven:**
  ...
  * WHEN a Storekeeper completes putaway for an `APPROVED` receipt, the system SHALL increase regular inventory by the approved actual quantity at the selected regular Bin location.
...
* **Scenario: Storekeeper completes putaway after approval**
  * Given a receipt has status `APPROVED` and 98 approved units
  * When Storekeeper selects a regular Bin with enough remaining capacity and completes putaway
  * Then the system SHALL set `receipt_items.location_id` and increase regular inventory by exactly 98 units at that Bin.
