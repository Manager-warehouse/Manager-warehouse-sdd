...
* **Event-driven:**
  ...
  * WHEN a Warehouse Staff submits QC sample results on a `DRAFT` receipt, the system SHALL:
    ...
    * Set `qc_result` status: `PENDING`/`PASSED`/`FAILED` only.
