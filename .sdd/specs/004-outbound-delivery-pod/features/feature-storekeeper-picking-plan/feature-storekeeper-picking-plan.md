...
* **Event-driven:**
  ...
  * WHEN a storekeeper changes a picking plan after goods have already been picked from the planned bin, the same `PUT /api/v1/delivery-orders/{id}/picking-plan` request SHALL require return-to-bin records in the payload before any new allocation is saved.
  ...
  * WHEN QC fail quantity requires replacement, the system SHALL allow the storekeeper to select replacement goods from the same FIFO-ranked valid regular inventory list.
...
* **State-driven:**
  * WHILE a Delivery Order is `PICKING` or later, the system SHALL block direct picking-plan changes unless the picked goods are first returned to their original locations through a recorded return-to-bin action.

## 4. API Endpoints

* `PUT /api/v1/delivery-orders/{id}/picking-plan` - Thu kho luu hoac dieu chinh ke hoach lay hang; neu hang da duoc pick, payload phai co `returnToBinRecords` de ghi nhan tra hang ve dung vi tri goc truoc khi luu plan moi.
...
* **Scenario: Change plan while waiting picking**
  ...
  * And warehouse staff has not started physical picking
...
* **Scenario: Require return to original bin before changing picked goods**
  * Given a delivery order is in `PICKING`
  * And goods have already been picked from an allocation that the revised plan removes or reduces
...
* **Scenario: Keep unchanged picked allocation without return**
  * Given a delivery order is in `PICKING`
  * And allocation A has already been picked but remains unchanged in the revised picking plan
...
* **Scenario: Save replacement after QC fail**
  ...
  * Then the system SHALL update the picking plan, store replacement history, reserve replacement goods, create a `PICKING_REPLACEMENT_SAVE` audit log, and require the replacement goods to be picked and QC checked.
