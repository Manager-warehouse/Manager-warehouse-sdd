...
* **Event-driven:**
  ...
  * WHEN the assigned driver confirms the vehicle has returned to the source warehouse, the system SHALL:
    ...
    * Keep any `RETURNED` goods in virtual `IN_TRANSIT` inventory until the separate return flow completes staff count/QC, Storekeeper approval, Storekeeper putaway planning, and staff putaway confirmation.
    * Create `COMPLETE_TRIP` audit log.

## 4. API Endpoints

* `GET /api/v1/trips` - Dispatcher xem danh sách chuyến xe trong các kho được gán, có thể lọc theo `warehouseId` và `status`.
* `GET /api/v1/trips/{id}` - Dispatcher/Trưởng kho/Admin/CEO xem chi tiết chuyến xe trong phạm vi kho được phép, gồm thông tin xe, tài xế, lịch trình và các Delivery Order trong chuyến.
...
### Trip detail response contract

`GET /api/v1/trips/{id}` SHALL return a `TripResponse` containing the trip identity/status, `tripType`, warehouse, planned/departed/completed timestamps, notes, total weight/volume, assigned Delivery Orders, and vehicle/driver detail fields needed by dispatch UI:

* `vehicleId`, `vehiclePlate`, `vehicleType`, `vehicleMaxWeightKg`, `vehicleMaxVolumeM3`
* `driverId`, `driverName`, `driverPhone`, `driverLicenseNumber`

The endpoint SHALL enforce role permission and warehouse scope. Drivers SHALL use the driver-scoped detail endpoint defined in the Driver Mobile feature instead of this dispatcher endpoint.
...
* Trip completion SHALL NOT receive returned goods into regular inventory; returned goods remain in virtual `IN_TRANSIT` until warehouse staff count/QC them, Storekeeper approves quantity/quality, Storekeeper creates a putaway plan, and warehouse staff confirms putaway completion.
