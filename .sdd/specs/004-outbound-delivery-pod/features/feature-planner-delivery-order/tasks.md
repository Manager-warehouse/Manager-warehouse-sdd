...
- [x] T007 Add repository query for unpaid dealer invoices overdue beyond the dealer's configured maximum debt days per order in `backend/src/main/java/com/wms/repository/InvoiceRepository.java`
...
- [x] T028 [US2] Return `CREDIT_HOLD` for dealer credit hold, credit limit exceeded, or unpaid invoice overdue beyond the dealer's configured maximum debt days per order in `backend/src/main/java/com/wms/service/impl/DeliveryOrderServiceImpl.java`
- [x] T029 [US2] Return `INSUFFICIENT_STOCK` without suggested alternative warehouses when the selected warehouse lacks enough valid availability in `backend/src/main/java/com/wms/service/impl/DeliveryOrderServiceImpl.java`
