...
2. **Xử lý chênh lệch điều chuyển:** Bất kỳ chênh lệch nào giữa số lượng xuất đi (`sent_qty`) và số lượng nhận thực tế (`received_qty`) tại kho đích phải được ghi nhận lý do rõ ràng (`discrepancy_reason`), tạo hồ sơ chênh lệch mở (`discrepancy_incidents`) và tự động tạo phiếu điều chỉnh (`adjustments` type = 'TRANSFER_DISCREPANCY') hoặc Audit trail tương ứng. Hồ sơ chênh lệch được CEO, Kế toán trưởng hoặc Trưởng kho có phạm vi liên quan chốt trách nhiệm/hướng xử lý; bước chốt này không tự sửa tồn kho.
3. **Exception path không được làm lệch tồn:** Các lỗi nhập liệu hoặc lỗi nghiệp vụ trong luồng điều chuyển phải bị chặn bằng message rõ ràng trước khi mutate tồn kho/trạng thái, trừ các nhánh nghiệp vụ có chủ đích như discrepancy hold. Phê duyệt thiếu tồn phải fail trước khi tạo reservation/allocation một phần; final receive phải validate cấu trúc putaway và cấu hình In-Transit/Quarantine trước khi chuyển tồn.
4. **Hiển thị lỗi nhất quán:** Frontend phải ưu tiên inline error cho ô nhập, hiển thị message backend đã dịch tiếng Việt qua toast gọn, chống trùng/chồng toast, và chỉ refresh dữ liệu sau khi mutation thành công.
...
* **Các User Stories:**
  ...
  * **US-WMS-23 (P2):** [Quản lý Danh mục Xe tải & Tài xế Nội bộ](../../.sdd/specs/002-master-data-management/features/feature-dispatcher-fleet-drivers.md) (xe: Rảnh/Đang đi chuyến/Bảo trì; tài xế: Rảnh/Đang đi chuyến/Không khả dụng). Xe chỉ quản lý tải trọng tối đa theo kg, không quản lý thể tích thùng xe trong Sprint 1. `ON_TRIP` là trạng thái do hệ thống cập nhật theo vòng đời chuyến; màn danh mục chỉ cho cập nhật trạng thái sẵn sàng/bảo trì/không khả dụng. Chỉ Dispatcher được bật/tắt `is_active` của xe/tài xế. Hồ sơ tài xế lấy họ tên từ tài khoản liên kết và gán chuyến bị chặn nếu chưa có hồ sơ hoặc GPLX hết hạn/thiếu hạn.
...
* **Các User Stories:**
  ...
  * **US-WMS-12 (P1):** [Lập, Duyệt và Xác nhận Phiếu Điều chuyển Kho](../../.sdd/specs/005-inter-warehouse-transfer/features/feature-storekeeper-transfer-ship.md). Trưởng kho nguồn duyệt thì khóa hàng ngay sau khi pre-check đủ tồn FIFO hợp lệ; nếu thiếu tồn thì fail không để lại reservation/allocation một phần. Mỗi phiếu điều chuyển gắn đúng một chuyến xe nội bộ riêng do Dispatcher lập; công nhân kho nguồn xếp hàng và báo số lượng thực xếp trước, Thủ kho nguồn QC xuất sau đó mới chốt số gửi/bàn giao tài xế; QC thất bại quay lại công nhân hạ/đổi/xếp lại và báo cáo lại trước khi QC lại. Tài xế xác nhận rời kho mới chuyển `IN_TRANSIT`. Kho đích nhận hàng theo 3 bước: công nhân nhập số lượng, Thủ kho kiểm/QC/chọn vị trí nhập, Trưởng kho xác nhận cuối; phần QC lỗi vào Quarantine, nhận thừa bị chặn, thiếu thì tạo `TRANSFER_DISCREPANCY` và hồ sơ chênh lệch mở để CEO/Kế toán trưởng/Trưởng kho xử lý sau. Chi tiết phiếu có panel bằng chứng ảnh read-only cho CEO/Trưởng kho/Kế toán trưởng, phân nhóm theo luồng xuất kho nguồn, bàn giao vận chuyển, QC kho nhận và quay đầu xe. (Xem thêm: [Kho Đích Tiếp nhận](../../.sdd/specs/005-inter-warehouse-transfer/features/feature-storekeeper-transfer-receive.md))
* **Actors:** Planner, Trưởng kho (Kho nguồn), Dispatcher, Công nhân/Nhân viên kho nguồn, Thủ kho (Kho nguồn), Nhân viên kho/Công nhân kho đích, Thủ kho (Kho đích), Trưởng kho (Kho đích), Nhân viên kho, Tài xế.
* **Endpoints chính:** `/api/v1/inter-warehouse-transfers`, `/api/v1/inter-warehouse-transfers/{id}`, `/api/v1/inter-warehouse-transfers/{id}/cancel`, `/api/v1/inter-warehouse-transfers/{id}/approve`, `/api/v1/inter-warehouse-transfers/{id}/reject`, `/api/v1/inter-warehouse-transfers/{id}/trip`, `/api/v1/inter-warehouse-transfers/{id}/source-load-report`, `/api/v1/inter-warehouse-transfers/{id}/outbound-qc`, `/api/v1/inter-warehouse-transfers/{id}/ship`, `/api/v1/inter-warehouse-transfers/{id}/unship`, `/api/v1/inter-warehouse-transfers/{id}/depart`, `/api/v1/inter-warehouse-transfers/{id}/receive-count`, `/api/v1/inter-warehouse-transfers/{id}/receive-check`, `/api/v1/inter-warehouse-transfers/{id}/final-receive`, `/api/v1/transfer-discrepancy-incidents`.
