# -*- coding: utf-8 -*-
"""
Generate result_test.md and fill test_final.xlsx based on Template3_System Test.xlsx
"""
...
MD_OUT = ROOT / 'docs' / 'test' / 'result_test.md'
...
def generate_markdown(all_data):
    md = []
    md.append('# BÁO CÁO KẾT QUẢ SYSTEM TEST (RESULT TEST)')
    md.append(f'**Dự án:** {PROJECT_NAME}')
    md.append(f'**Mã dự án:** {PROJECT_CODE}')
    md.append(f'**Ngày lập báo cáo:** {RUN_DATE}')
    md.append(f'**Tham chiếu Template:** Template3_System Test.xlsx')
    md.append('\n---\n')

    md.append('## 1. TỔNG QUAN VỀ SYSTEM TEST (TEST STATISTICS)')
    md.append('| STT | Mã Module | Tên Module | Passed | Failed | Pending | N/A | Total TCs |')
    md.append('|---|---|---|---|---|---|---|---|')

    total_p = total_f = total_pend = total_na = total_tc = 0
    for i, mod_data in enumerate(all_data, start=1):
        p = sum(1 for t in mod_data['tcs'] if t['r1'] == 'Passed')
        f = sum(1 for t in mod_data['tcs'] if t['r1'] == 'Failed')
        na = sum(1 for t in mod_data['tcs'] if t['r1'] == 'N/A')
        pend = sum(1 for t in mod_data['tcs'] if t['r1'] == 'Pending')
        tot = len(mod_data['tcs'])
        ...
        md.append(f"| {i} | {mod_data['code']} | {mod_data['name']} | {p} | {f} | {pend} | {na} | {tot} |")

    md.append(f"| **TỔNG** | | | **{total_p}** | **{total_f}** | **{total_pend}** | **{total_na}** | **{total_tc}** |")
    
    coverage = round((total_p + total_f) * 100 / (total_tc - total_na or 1), 1)
    pass_rate = round(total_p * 100 / (total_tc - total_na or 1), 1)
    md.append(f"\n- **Độ phủ kiểm thử (Test Coverage):** {coverage}%")
    md.append(f"- **Tỷ lệ Pass (Test Successful Coverage):** {pass_rate}%")
    md.append('\n---\n')

    md.append('## 2. CHI TIẾT CÁC TEST CASES THEO MODULE (TEST CASES DETAILS)')

    for mod_data in all_data:
        md.append(f"\n### Module [{mod_data['code']}] - {mod_data['name']}")
        md.append(f"**Yêu cầu kiểm thử:** {mod_data['req']}\n")
        md.append('| Test Case ID | Scenario | Description | Procedure | Expected Results | Pre-conditions | Round 1 Status | Test Date | Tester | Note |')
        md.append('|---|---|---|---|---|---|---|---|---|---|')

        for tc in mod_data['tcs']:
            proc_clean = tc['proc'].replace('\n', '<br>')
            exp_clean = tc['exp'].replace('\n', '<br>')
            pre_clean = tc['pre'].replace('\n', '<br>')
            md.append(f"| {tc['id']} | {tc['scenario']} | {tc['desc']} | {proc_clean} | {exp_clean} | {pre_clean} | {tc['r1']} | {tc['dt']} | {tc['tester']} | {tc['note']} |")

    MD_OUT.parent.mkdir(parents=True, exist_ok=True)
    MD_OUT.write_text('\n'.join(md), encoding='utf-8')
    print(f"Successfully generated {MD_OUT}")

def fill_excel_final(all_data):
    try:
        import openpyxl
    except ImportError:
        return
    ...
    print(f"Loaded {EXCEL_OUT}. Sheets: {wb.sheetnames}")
    ...
    for i, mod in enumerate(all_data, start=1):
        ...
        ws_tc.cell(row=r, column=4).value = mod['code']
        ws_tc.cell(row=r, column=5).value = mod['req']

    # 3. Setup module sheets
    ...
    mod_sheets = [ws1.title, ws2.title]

    for mod in all_data[2:]:
        new_ws = wb.copy_worksheet(wb[all_data[1]['code']])
        new_ws.title = mod['code']
        mod_sheets.append(new_ws.title)

    # Fill data for each module sheet
    scenario_fill = openpyxl.styles.PatternFill('solid', fgColor='BDD7EE')
    scenario_font = openpyxl.styles.Font(name='Calibri', bold=True, size=10, color='1F3864')
    status_fill = {
        'Passed': openpyxl.styles.PatternFill('solid', fgColor='C6EFCE'),
        'Failed': openpyxl.styles.PatternFill('solid', fgColor='FFC7CE'),
        'Pending': openpyxl.styles.PatternFill('solid', fgColor='FFEB9C'),
        'N/A': openpyxl.styles.PatternFill('solid', fgColor='D9D9D9'),
    }
    ...
    for mod in all_data:
        ...
        for tc in mod['tcs']:
            if tc['scenario'] != current_scenario:
                ...
                sc_cell.font = scenario_font
                sc_cell.fill = scenario_fill
                current_row += 1
            ...
            c6.value = r1_st
            if r1_st in status_fill:
                c6.fill = status_fill[r1_st]
            ...
            c9.value = 'Pending'
            c9.fill = status_fill['Pending']

            c12 = ws.cell(row=r, column=12)
            c12.value = 'Pending'
            c12.fill = status_fill['Pending']
    ...
    for i, mod in enumerate(all_data, start=1):
        ...
        ws_stat.cell(row=row, column=3).value = f'={safe}!B1'
        ws_stat.cell(row=row, column=4).value = f'={safe}!B5'
        ws_stat.cell(row=row, column=5).value = f'={safe}!C5'
        ws_stat.cell(row=row, column=6).value = f'={safe}!D5'
        ws_stat.cell(row=row, column=7).value = f'={safe}!E5'
        ws_stat.cell(row=row, column=8).value = f'={safe}!B3'

    sub_row = 11 + len(all_data)
    ws_stat.cell(row=sub_row, column=3).value = 'Sub total'
    ws_stat.cell(row=sub_row, column=4).value = f'=SUM(D11:D{sub_row-1})'
    ws_stat.cell(row=sub_row, column=5).value = f'=SUM(E11:E{sub_row-1})'
    ws_stat.cell(row=sub_row, column=6).value = f'=SUM(F11:F{sub_row-1})'
    ws_stat.cell(row=sub_row, column=7).value = f'=SUM(G11:G{sub_row-1})'
    ws_stat.cell(row=sub_row, column=8).value = f'=SUM(H11:H{sub_row-1})'
    ...
    print(f"Successfully updated {EXCEL_OUT}")

def main():
    print("Parsing Surefire reports...")
    ...
    print("Generating Markdown report: result_test.md...")
    generate_markdown(all_data)

    print("Filling Excel template copy: test_final.xlsx...")
    fill_excel_final(all_data)
