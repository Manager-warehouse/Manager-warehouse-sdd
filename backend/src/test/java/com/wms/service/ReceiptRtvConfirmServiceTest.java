 ...
 *
 * <p>
 * Covers: RTV confirm full quantity happy path, partial quantity mismatch (HTTP
 * 422),
 * duplicate confirmation (HTTP 409), quarantine inventory non-negative
 * invariant.
 * </p>
 */
...
class ReceiptRtvConfirmServiceTest {
        ...
        void confirmRtv_fullQuantity_deductsQuarantineInventory() {
                ...
                verify(adjustmentRepository)
                                .save(argThat(adj -> adj.getApprovedBy() != null && adj.getApprovedAt() != null));
        }
        ...
        void confirmRtv_happyPath_auditLogContainsDeductedQty() {
                ...
                when(adjustmentRepository.findConfirmedRtvByReference(any(), any(), any()))
                                .thenReturn(Optional.empty());
                when(adjustmentRepository.findPendingRtvByReference(any(), any(), any()))
                                .thenReturn(Optional.of(pendingRtv));
                when(receiptItemRepository.findByReceiptId(1L)).thenReturn(List.of(failedItem));
