...
class SplitDeliveryPlanControllerTest {
    ...
    void confirmDealerArrival_returnsSplitLegProgress() throws Exception {
        when(currentUserService.getRequiredCurrentUser()).thenReturn(driver);
        when(splitDeliveryPlanService.confirmDealerArrival(10L, driver))
                .thenReturn(milestone(false, false));

        mockMvc.perform(put("/api/v1/split-delivery-plans/10/dealer-arrival").with(csrf()))
                .andExpect(status().isOk())
    ...
    void confirmHandover_returnsLeadPodOtpGate() throws Exception {
        when(currentUserService.getRequiredCurrentUser()).thenReturn(driver);
        when(splitDeliveryPlanService.confirmHandover(10L, driver))
                .thenReturn(milestone(true, true));

        mockMvc.perform(put("/api/v1/split-delivery-plans/10/handover").with(csrf()))
                .andExpect(status().isOk())
    ...
    void failDeliveryLeg_requiresReasonAndReturnsWholePlanState() throws Exception {
        when(currentUserService.getRequiredCurrentUser()).thenReturn(driver);
        when(splitDeliveryPlanService.failDelivery(eq(10L), any(), eq(driver)))
                .thenReturn(milestone(false, false));

        mockMvc.perform(put("/api/v1/split-delivery-plans/10/fail-delivery")
                        .with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
    ...
    @Test
    @WithMockUser(username = "driver@wms.com", roles = "DRIVER")
    void completePlan_releasesWholeConvoyThroughLeadRoute() throws Exception {
        when(currentUserService.getRequiredCurrentUser()).thenReturn(driver);
        when(splitDeliveryPlanService.completePlan(eq(10L), any(), eq(driver)))
                .thenReturn(milestone(true, true));

        mockMvc.perform(put("/api/v1/split-delivery-plans/10/complete")
                        .with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{}"))
                .andExpect(status().isOk())
