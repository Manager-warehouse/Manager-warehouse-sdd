...
ALTER TABLE receipt_items
    ALTER COLUMN expected_qty TYPE
INTEGER
...
CREATE UNIQUE INDEX
IF NOT EXISTS ux_receipts_purchase_supplier_warehouse_source
    ON receipts
(supplier_id, warehouse_id, source_order_code)
    WHERE type = 'PURCHASE'
      AND status <> 'RETURNED_TO_SUPPLIER'
      AND source_order_code IS NOT NULL;

CREATE TABLE
IF NOT EXISTS document_sequences
(
    sequence_key VARCHAR
(50) PRIMARY KEY,
    next_value BIGINT NOT NULL CHECK
(next_value > 0),
