ALTER TABLE split_delivery_legs
    ADD COLUMN dealer_arrived_at TIMESTAMPTZ,
    ADD COLUMN handover_confirmed_at TIMESTAMPTZ,
    ADD COLUMN failure_reported_at TIMESTAMPTZ,
    ADD COLUMN failure_reason TEXT;

ALTER TABLE deliveries
    ADD COLUMN goods_image_object_key VARCHAR(500),
    ADD COLUMN goods_image_original_filename VARCHAR(255),
    ADD COLUMN goods_image_content_type VARCHAR(100),
    ADD COLUMN goods_image_size_bytes BIGINT CHECK (goods_image_size_bytes >= 0),
    ADD COLUMN goods_image_uploaded_at TIMESTAMPTZ,
    ADD COLUMN signed_document_object_key VARCHAR(500),
    ADD COLUMN signed_document_original_filename VARCHAR(255),
    ADD COLUMN signed_document_content_type VARCHAR(100),
    ADD COLUMN signed_document_size_bytes BIGINT CHECK (signed_document_size_bytes >= 0),
    ADD COLUMN signed_document_uploaded_at TIMESTAMPTZ;
...
WHERE issued_at IS NULL AND status = 'ACTIVE';

CREATE INDEX idx_split_delivery_legs_arrival
    ON split_delivery_legs(split_plan_id, dealer_arrived_at);

CREATE INDEX idx_split_delivery_legs_handover
    ON split_delivery_legs(split_plan_id, handover_confirmed_at);
